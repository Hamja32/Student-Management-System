...
.stat-info p { margin: 0; color: #777; font-size: 14px; text-transform: uppercase; }
</style>
</head>
<body>
<%
...
<div class="nav-bar" style="display: flex; justify-content: space-between; align-items: center;">
    <div>
        <span style="font-weight: bold; color: #2c3e50;">Welcome, ${sessionScope.currentUser.email} (Teacher)</span>
    </div>
    
    <div>
        <a href="<%=request.getContextPath() %>/list" class="btn-nav">Refresh List</a>
        <a href="<%= request.getContextPath()%>/new" class="btn-nav btn-add">+ Add Student</a>
        <a href="login" class="btn-nav" style="background-color: #e74c3c;">Logout</a>
    </div>
</div>
<div class="container">
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><span style="background:#e8f4f8; color:#007bff; padding:3px 8px; border-radius:10px; font-size:12px;"><%= student.getCourse() %></span></td>
                
                <td class="action-links">
                    <a href="<%= request.getContextPath() %>/edit?id=<%= student.getId() %>" class="btn-edit">Edit</a>
                    <a href="<%= request.getContextPath() %>/view?id=<%= student.getId() %>" class="btn-view">Result</a>
                    <a href="<%= request.getContextPath() %>/delete?id=<%= student.getId() %>" class="btn-delete" onclick="return confirm('Are you sure you want to delete this student?');">Delete</a>
                </td>
            </tr>
            <%
                }
