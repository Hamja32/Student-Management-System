...
public class ControllerServlet extends HttpServlet {
	...
	// dashboard
	// Route: "/dashboard" (Ise default home page bana sakte hain)
	private void showDashboard(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {

		// 1. Data fetch karo
		int totalStudents = dashboardDAO.getTotalStudents();
		int totalPass = dashboardDAO.getPassCount();

		// Total Fail = Total Students - Total Pass (Simple Math)
		// Note: Ye tabhi sahi chalega agar sabka result declared hai.
		// Abhi ke liye maan lete hain jinhone exam diya wahi count ho rahe hain.
		int totalFail = totalStudents - totalPass;
		if (totalFail < 0)
			totalFail = 0; // Negative number na aaye

		// 2. Request me set karo
		request.setAttribute("totalStudents", totalStudents);
		request.setAttribute("passCount", totalPass);
		request.setAttribute("failCount", totalFail);
		// --- 2. NEW: Top 3 Students ---
	    List<TopStudent> toppers = dashboardDAO.getTop3Students();
	    request.setAttribute("top3List", toppers); // JSP ke liye set kiya
		// 3. Dashboard JSP par bhejo
